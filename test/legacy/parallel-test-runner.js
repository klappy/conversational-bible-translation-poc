...
async function runParallelTests(baseUrl = "http://localhost:9999", batchSize = 3) {
  ...
  for (let i = 0; i < testConfigs.length; i += batchSize) {
    ...
    console.log(
      `\n📦 Running batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(
        testConfigs.length / batchSize
      )}`
    );
...
function generateSummaryReport(results, totalDuration) {
  ...
  console.log(
    `   Average Test Duration: ${Math.round(
      results.reduce((sum, r) => sum + (r.duration || 0), 0) / results.length / 1000
    )}s`
  );
  ...
  console.log(`\n⚡ Performance Metrics:`);
  console.log(
    `   Parallel Speedup: ${Math.round(((results.length * 30) / totalDuration) * 1000)}x`
  );
  console.log(`   Tests per minute: ${Math.round(results.length / (totalDuration / 60000))}`);

  // Quick response analysis
  const suggestionsOnlySuccess =
    byMode.suggestions.filter((r) => r.success).length / byMode.suggestions.length;
  ...
  if (suggestionsOnlySuccess < manualOnlySuccess) {
    console.log(
      `   ⚠️  Suggestions mode has lower success rate - quick responses may need improvement`
    );
  }
}
...
if (process.argv[1] === import.meta.url) {
  ...
  switch (mode) {
    ...
    default:
      console.log(
        "Usage: node parallel-test-runner.js [parallel|suggestions|manual|mixed|quick] [baseUrl]"
      );
      process.exit(1);
  }
}
