...
const regressionTests = [
  {
    ...
    async run() {
      ...
      let response = await makeRequest("/conversation", "POST", 
        { message: "", history: [] }, sessionId);
      
      // Provide name
      response = await makeRequest("/conversation", "POST",
        { message: "Chris", history: [] }, sessionId);
      
      // Check suggestions are for CURRENT question (language), not name
      const suggestions = response.suggestions || [];
      const hasLanguageSuggestion = suggestions.some(s => 
  ...
  
  {
    ...
    async run() {
      ...
      await makeRequest("/conversation", "POST", 
        { message: "Test User", history: [] }, sessionId);
      await makeRequest("/conversation", "POST",
        { message: "English", history: [] }, sessionId);
      await makeRequest("/conversation", "POST",
        { message: "English", history: [] }, sessionId);
      await makeRequest("/conversation", "POST",
        { message: "Simple English", history: [] }, sessionId);
      await makeRequest("/conversation", "POST",
        { message: "Kids", history: [] }, sessionId);
      await makeRequest("/conversation", "POST",
        { message: "Grade 3", history: [] }, sessionId);
      await makeRequest("/conversation", "POST",
        { message: "Fun", history: [] }, sessionId);
      await makeRequest("/conversation", "POST",
        { message: "Meaning-based", history: [] }, sessionId);
      
      // Now in understanding - provide phrase explanations
      await makeRequest("/conversation", "POST",
        { message: "Continue", history: [] }, sessionId);
      await makeRequest("/conversation", "POST",
        { message: "Time before kings ruled", history: [] }, sessionId);
      ...
      if (hasGlossary) {
        return { 
          passed: true, 
          message: `Collected ${Object.keys(state.glossary?.keyTerms || {}).length} terms, ${Object.keys(state.glossary?.userPhrases || {}).length} phrases` 
        };
      } else {
  ...
  
  {
    ...
    async run() {
      ...
      const quickSetup = [
        "Test User", "English", "English", "Simple English", 
        "Kids", "Grade 3", "Fun", "Meaning-based",
        "Continue", "Time before kings", "No food", "Guy from Bethlehem",
        "Went to another place", "His family", "Ready to draft"
      ];
      ...
      await makeRequest("/conversation", "POST",
        { message: draft, history: [] }, sessionId);
  ...
  
  {
    ...
    async run() {
      ...
      // Set user name
      await makeRequest("/conversation", "POST",
        { message: "TestUserName", history: [] }, sessionId);
      
      // Set a few more settings
      await makeRequest("/conversation", "POST",
        { message: "Spanish", history: [] }, sessionId);
      await makeRequest("/conversation", "POST",
        { message: "English", history: [] }, sessionId);
