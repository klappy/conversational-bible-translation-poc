...
async function listAllSessions(baseUrl = "http://localhost:9999") {
  try {
    const response = await fetch(`${baseUrl}/.netlify/functions/canvas-state/sessions`, {
      method: "GET",
    });
...
async function deleteSession(baseUrl, sessionKey) {
  try {
    ...
    const response = await fetch(`${baseUrl}/.netlify/functions/canvas-state/session`, {
      method: "DELETE",
      headers: {
...
export async function resetDefaultSession(baseUrl = "http://localhost:9999") {
  try {
    const response = await fetch(`${baseUrl}/.netlify/functions/canvas-state?reset=true`, {
      method: "GET",
      headers: {
