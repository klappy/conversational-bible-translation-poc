...
async function sendAndCheck(turn, message, history, expectedInQuestion, expectedInSuggestions) {
  ...
  const question =
    result.messages.find((m) => m.agent?.name === "Translation Assistant")?.content || "";
  const suggestions = result.suggestions || [];

  console.log(`   Question: "${question.substring(0, 70)}..."`);
  console.log(`   Suggestions: ${JSON.stringify(suggestions)}`);

  const qMatch = expectedInQuestion
    ? question.toLowerCase().includes(expectedInQuestion.toLowerCase())
    : true;
  const sMatch = expectedInSuggestions
    ? suggestions.some((s) => s.toLowerCase().includes(expectedInSuggestions.toLowerCase()))
    : true;
